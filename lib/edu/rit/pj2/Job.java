 ...
 * <P>
 * If the job cannot set up a connection to the Tracker host and port, the job
 * falls back to executing all tasks in the same process as the job itself. In
 * this case there is no coordination among separate jobs. For further
 * information about the Tracker, see package {@linkplain edu.rit.pj2.tracker
 * edu.rit.pj2.tracker}.
 * <P>
...
// Exported operations.
	 ...
	 * property specifies the host name at which the Tracker is listening for
	 * connections. If not set, the default is to try to connect to the Tracker
	 * on each of the local machine's network interfaces, stopping as soon as a
