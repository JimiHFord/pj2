...
// Unit:    Class edu.rit.util.LongList
//
...
/**
 * Class LongList provides a list of long integers (type <TT>long</TT>) stored
...
// Exported operations.
	...
	public long get
		...
		if (0 > p || p >= size)
			...
			throw new IndexOutOfBoundsException
				("LongList.get(): p = "+p+" out of bounds");
			}
		return item[p];
		}
	...
	public void set
		...
		if (0 > p || p >= size)
			{
			throw new IndexOutOfBoundsException
				("LongList.set(): p = "+p+" out of bounds");
			}
		item[p] = i;
		}
	...
	public void swap
		...
		if (0 > p || p >= size)
			{
			throw new IndexOutOfBoundsException
				("LongList.swap(): p = "+p+" out of bounds");
			}
		if (0 > q || q >= size)
			{
			throw new IndexOutOfBoundsException
				("LongList.swap(): q = "+q+" out of bounds");
			}
		long tmp = item[p];
		item[p] = item[q];
		item[q] = tmp;
	...
	public long addLast
		...
		if (size == item.length)
			{
			long[] newitem = new long [item.length + CHUNK];
	...
	public long add
		...
		if (0 > p || p > size)
			{
			throw new IndexOutOfBoundsException
				("LongList.add(): p = "+p+" out of bounds");
			}
		if (size == item.length)
			{
			long[] newitem = new long [item.length + CHUNK];
	...
	public long remove
		...
		if (0 > p || p >= size)
			{
			throw new IndexOutOfBoundsException
				("LongList.remove(): p = "+p+" out of bounds");
			}
		-- size;
		long tmp = item[p];
	 ...
	 *     (unchecked exception) Thrown if <TT>p</TT> is out of bounds.
	 */
	public long swapRemoveLast
		(int p)
		{
		if (0 > p || p >= size)
			{
			throw new IndexOutOfBoundsException
				("LongList.swapRemoveLast(): p = "+p+" out of bounds");
			}
		-- size;
		long tmp = item[p];
		item[p] = item[size];
