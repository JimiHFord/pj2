...
// Exported operations.
	...
	public static int searchUnsorted
		...
		for (int i = 0; i < x.length; ++ i)
			{
			if (helper.compare (x[i], a) == 0) return i;
			}
		return -1;
		}
	...
	public static int searchInterval
		...
		int lo = 0;
		if (helper.compare (x[lo], a) > 0) return 0;

		int hi = x.length - 1;
		if (helper.compare (x[hi], a) <= 0) return x.length;
	...
	public static <T> int searchUnsorted
		...
		for (int i = 0; i < x.length; ++ i)
			{
			if (comp.compare (x[i], a) == 0) return i;
			}
		return -1;
		}
	...
	public static <T> int searchInterval
		...
		int lo = 0;
		if (comp.compare (x[lo], a) > 0) return 0;

		int hi = x.length - 1;
		if (comp.compare (x[hi], a) <= 0) return x.length;
	 ...
	 *          returned.
	 */
	public static <T extends Comparable> int searchUnsorted
		(T[] x,
		 T a)
		{
		for (int i = 0; i < x.length; ++ i)
			{
			if (x[i].compareTo (a) == 0) return i;
			}
		return -1;
		}
	 ...
	 *          returned.
	 */
	public static <T extends Comparable> int searchSorted
		(T[] x,
		 T a)
		{
		// Establish loop invariant.
	...
	/**
	 * Search the given ordered array of type <TT>T[]</TT> for an interval
	 * contining the given element. The array element's natural ordering
	 * (<TT>compareTo()</TT> method) is used to compare elements for order and
	 * equality. It is assumed that the array is sorted in the order determined
	 * by the natural ordering; otherwise, the <TT>searchInterval()</TT>
	 * method's behavior is not specified. An <I>O</I>(log <I>n</I>) binary
	 * search algorithm is used.
	 *
	 * @param  <T>     Array element data type.
	 ...
	 *     length, then <TT>i</TT> = &minus;1 is returned.
	 */
	public static <T extends Comparable> int searchInterval
		(T[] x,
		 T a)
		{
		// Establish loop invariant.
		...
		int lo = 0;
		if (x[lo].compareTo (a) > 0) return 0;

		int hi = x.length - 1;
		if (x[hi].compareTo (a) <= 0) return x.length;

		// Loop invariant: x[lo] <= a and a < x[hi].
		while (hi - lo > 1)
			{
			int mid = (hi + lo)/2;
			if (x[mid].compareTo (a) <= 0)
				lo = mid;
			else
				hi = mid;
			}
...
//		System.out.printf ("searchInterval() returns %d%n",
//			Searching.searchInterval (x, a, helper));
//		}
//
//	/**
