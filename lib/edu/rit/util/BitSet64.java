...
// Unit:    Class edu.rit.util.BitSet64
//
...
/**
 * Class BitSet64 provides a set of integers from 0 to 63. The set elements are
 * stored in a bitmap representation.
 * <P>
 * The bitmap representation is a value of type <TT>long</TT>. Bit 0 of the
 ...
 * <P>
 * <I>Note:</I> Class BitSet64 is not multiple thread safe.
 *
...
// Exported operations.
	...
	public boolean contains
		(int elem)
		{
		return (bitmap & maskForElement (elem)) != 0L;
		}
	 ...
	 * @return  This set.
	 */
	public BitSet64 symmetricDifference
		(BitSet64 set)
		{
		this.bitmap ^= set.bitmap;
		return this;
...
// Hidden operations.
	 ...
	 * elsewhere.
	 */
	private static long maskForElement
		(int elem)
		{
		return 0 <= elem && elem <= 63 ? 1L << elem : 0L;
		}
	 ...
	 */
	private static long maskForRange
		(int lb,
		 int ub)
		{
		lb = Math.max (lb, 0);
		ub = Math.min (ub, 64);
		if (lb >= ub) return 0L;
		long lbfill = lb == 64 ? 0xffffffffffffffffL : (1L << lb) - 1L;
		long ubfill = ub == 64 ? 0xffffffffffffffffL : (1L << ub) - 1L;
