...
// Unit:    Class edu.rit.util.FloatList
//
...
/**
 * Class FloatList provides a list of single precision floating point numbers
 * (type <TT>float</TT>) stored in a dynamically-sized array.
 * <P>
...
// Exported operations.
	...
	public float get
		...
		if (0 > p || p >= size)
			...
			throw new IndexOutOfBoundsException
				("FloatList.get(): p = "+p+" out of bounds");
			}
		return item[p];
		}
	...
	public void set
		...
		if (0 > p || p >= size)
			{
			throw new IndexOutOfBoundsException
				("FloatList.set(): p = "+p+" out of bounds");
			}
		item[p] = i;
		}
	...
	public void swap
		...
		if (0 > p || p >= size)
			{
			throw new IndexOutOfBoundsException
				("FloatList.swap(): p = "+p+" out of bounds");
			}
		if (0 > q || q >= size)
			{
			throw new IndexOutOfBoundsException
				("FloatList.swap(): q = "+q+" out of bounds");
			}
		float tmp = item[p];
		item[p] = item[q];
		item[q] = tmp;
	 ...
	 */
	public float addLast
		(float i)
		{
		if (size == item.length)
			{
			float[] newitem = new float [item.length + CHUNK];
	...
	public float add
		...
		if (0 > p || p > size)
			{
			throw new IndexOutOfBoundsException
				("FloatList.add(): p = "+p+" out of bounds");
			}
		if (size == item.length)
			{
			float[] newitem = new float [item.length + CHUNK];
	...
	public float remove
		...
		if (0 > p || p >= size)
			{
			throw new IndexOutOfBoundsException
				("FloatList.remove(): p = "+p+" out of bounds");
			}
		-- size;
		float tmp = item[p];
	 ...
	 *     (unchecked exception) Thrown if <TT>p</TT> is out of bounds.
	 */
	public float swapRemoveLast
		(int p)
		{
		if (0 > p || p >= size)
			{
			throw new IndexOutOfBoundsException
				("FloatList.swapRemoveLast(): p = "+p+" out of bounds");
			}
		-- size;
		float tmp = item[p];
		item[p] = item[size];
